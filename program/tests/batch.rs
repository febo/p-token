...
fn batch_instruction(instructions: Vec<Instruction>) -> Result<Instruction, ProgramError> {
    ...
    for instruction in instructions {
        ...
        // Number of accounts of the instruction.
        data.push(instruction.accounts.len() as u8);

        for account in instruction.accounts {
            let index = accounts.iter().position(|x| x.pubkey == account.pubkey);

            let index = if let Some(index) = index {
                // The account is already in the list.
                let existing = accounts.get_mut(index).unwrap();
                ...
                // The account is not in the list.
...
#[test_case::test_case(TOKEN_PROGRAM_ID ; "p-token")]
#[tokio::test]
async fn sequence(token_program: Pubkey) {
    let mut context = ProgramTest::new("token_program", token_program, None)
        .start_with_context()
        .await;
